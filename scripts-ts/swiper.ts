...
function createSlideComponent(imgSrc: string): ImageElement {
  const slide = document.createElement("img") as ImageElement;
  slide.src = imgSrc;
  slide.classList.add(
    "absolute",
    "w-full",
...
function createThumbComponent(imgSrc: string): ThumbElement {
  const thumb = document.createElement("div") as ThumbElement;
...
function swiper({ title, showFraction = true, images, startIndex = 0 }: Args) {
  ...
  const slides: ImageElement[] = [];
  const thumbs: ThumbElement[] = [];
